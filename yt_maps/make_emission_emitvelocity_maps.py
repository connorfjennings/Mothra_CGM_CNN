...
def _Ha_cloudy(field, data):
    ...
    T = np.where(T.value<1e4,1e4,T.value)
...
def _emittinggas(field,data):
    L  = data["gas","Halpha_brightness"].to_value("erg/s/cm**3/arcsec**2")
    return (L > 5e-43).astype("float64")
ds.add_field(
    name=("gas", "emitting"),
...
def _emitting_density(field,data): #this is used only as a weight
    return data[('gas','density')]*data[("gas", "emitting")]
ds.add_field(
...
field_list = [("gas","emitting_velocity_u"),
              ("gas","emitting_velocity_v"),
              ("gas","emitting_velocity_w")]
...
for field in field_list:
    p = yt.OffAxisProjectionPlot(
        ...
        north_vector=v_hat,
        weight_field=("gas","emitting_density"),
...
outdir = '/home/cj535/palmer_scratch/TNG50_cutouts/MW_sample_maps/Halphavelocity/'
field_list = [("gas","velocity_u"),
              ("gas","velocity_v"),
              ("gas","velocity_w")]
for field in field_list:
    p = yt.OffAxisProjectionPlot(
        ...
        center=center_prop, width=width_proper,
        north_vector=v_hat,
        weight_field=("gas","Halpha_emissivity"),
        buff_size=resolution,
        data_source=sp
    )
    img = p.frb[field]
    outfile = f'{basefilename}_view{angle_number:02d}_Halpha_{field[1]}.npy'
    outpath = outdir+outfile
    np.save(outpath,img.to_value(img.units).astype(np.float32))
