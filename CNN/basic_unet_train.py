...
def save_checkpoint(path, model, opt, scaler, epoch, val_loss, extra=None):
    payload = {
    ...
    if extra:  # e.g., mean/std, config, etc.
...
def main():
    ...
    # 5) model/optim
    model = UNetSmall(in_ch=3, out_ch=2, base=64).to(DEVICE)
    ...
    best_val = float("inf")
    best_path = "checkpoints/unet_cgm_best.pt"
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if va_loss < best_val:
            best_val = va_loss
            save_checkpoint(
                best_path, model, opt, scaler, epoch, va_loss,
                extra={"mean": mean, "std": std,
                       "config": {"R_MASK": R_MASK, "H": H, "W": W,
        ...
        if epoch % 10 == 0:
            periodic_path = f"checkpoints/unet_cgm_epoch{epoch:03d}.pt"
            save_checkpoint(
                periodic_path, model, opt, scaler, epoch, va_loss,
                extra={"mean": mean, "std": std,
                       "config": {"R_MASK": R_MASK, "H": H, "W": W,
                                  "BATCH_SIZE": BATCH_SIZE, "LR": LR}}
            )
